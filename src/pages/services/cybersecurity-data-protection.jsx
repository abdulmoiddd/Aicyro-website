...
import LeadMagnet from "@/Components/Home/SecurityChecklistTeaser";
...
const CyberSecurity = () => {
  ...
  useEffect(() => {
    // Keeping the path for Cybersecurity
    const serviceRef = ref(database, "Servicessubpages/Cybersecurity");
  ...
  // Helper to format text with bullet points AND Numbered lists
      ...
      <React.Fragment key={index}>
        ...
        {line.trim().startsWith("•") || line.trim().startsWith("✔") ? (
          ...
          <span className="block pl-4 mb-2 text-[var(--foreground-muted)]/90">
            {/* Updated Bullet Color using variable */}
            <span className="text-[var(--accent-blue)] mr-2">
              {line.trim().charAt(0)}
            </span>
            {line.trim().substring(1)}
          </span>
        ) : /* Numbered Lists (e.g., "1. Zero-Trust") */
        /^[0-9]+\./.test(line.trim()) ? (
          // UPDATED: Numbered List Color
          <span className="block mt-6 mb-2 text-lg font-bold text-[var(--accent-blue)]">
            {line.trim()}
          </span>
        ) : (
          /* Standard Paragraph */
    ...
    <div className="bg-[var(--background)] min-h-screen text-[var(--foreground)] relative font-sans selection:bg-[var(--accent-blue)] selection:text-white transition-colors duration-300">
      ...
      <div className="pt-32 pb-24 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
        ...
        >
          ...
          >
            The 2025 Cybersecurity <br />
            {/* Updated Gradient to match SaaS Page using vars */}
          ...
          {/* DIAGRAM TRIGGER: Visualizing the threat landscape */}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 relative">
                  ...
                  serviceData.Sections.map((section, idx) => (
                    <li key={idx}>
                      <button
                        ...
                        // Updated Hover Color and Text Color
          ...
          <main className="lg:col-span-9 space-y-12">
            ...
            {/* DIAGRAM TRIGGER: Visualizing the architecture typically discussed in sections (Zero Trust) */}
            <div className="mb-8">
...
[Image of Zero Trust security architecture diagram]
            ...
            {serviceData.Sections &&
              serviceData.Sections.map((section, idx) => (
                <motion.section
                  ...
                  // Updated Border and Card Background
      ...
      {/* --- BACKGROUND GLOWS (Updated to Purple/Blue match variables) --- */}
      <div className="fixed inset-0 z-0 pointer-events-none overflow-hidden">
        <div className="absolute top-0 left-[-20%] w-[50vw] h-[50vw] bg-[var(--primary)]/20 rounded-full blur-[120px]" />
        <div className="absolute bottom-0 right-[-20%] w-[50vw] h-[50vw] bg-[var(--accent-blue)]/20 rounded-full blur-[120px]" />
      </div>
