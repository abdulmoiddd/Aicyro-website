...
// --- MOCK DATA FOR CHARTS (Privacy/Security Context) ---
const threatData = [
  { name: "00:00", threats: 12, secured: 12 },
  { name: "04:00", threats: 18, secured: 18 },
  { name: "08:00", threats: 45, secured: 44 },
  { name: "12:00", threats: 80, secured: 80 },
  { name: "16:00", threats: 55, secured: 55 },
  { name: "20:00", threats: 30, secured: 30 },
  { name: "23:59", threats: 20, secured: 20 },
];

const encryptionStats = [
  { name: "Standard", value: 40 }, // Security Score
  { name: "Shady Protocol", value: 99 }, // Security Score
];
...
const Shady = () => {
  ...
  // --- FETCH DATA ---
  useEffect(() => {
    // Pointing to "Protfolio/Shady"
    const portfolioRef = ref(database, "Protfolio/Shady");
    ...
    const unsubscribe = onValue(
      ...
      (snapshot) => {
        if (snapshot.exists()) {
          ...
          console.warn("No data found at 'Protfolio/Shady'");
        ...
        <div className="flex flex-col items-center gap-6 z-10">
          <div className="relative">
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <EyeOff className="w-6 h-6 text-[#677ED6] opacity-50" />
            </div>
          </div>
  ...
  return (
    <div className="bg-[#0A0118] text-white min-h-screen relative font-sans selection:bg-[#677ED6]/30">
      <Seo
        title={data.title || "Shady Project"}
        description={
          data.overview ? data.overview.slice(0, 160) : "Privacy solutions."
        }
        url="/portfolio/shady"
      />
      ...
      <div className="fixed inset-0 z-0 overflow-hidden pointer-events-none">
        <div className="absolute top-[-20%] left-[-10%] w-[600px] h-[600px] bg-slate-800/20 rounded-full blur-[120px]" />
        <div className="absolute bottom-[20%] right-[-10%] w-[500px] h-[500px] bg-[#677ED6]/10 rounded-full blur-[100px]" />
        {/* Matrix-like subtle grid */}
        <div className="absolute inset-0 bg-[linear-gradient(rgba(103,126,214,0.03)_1px,transparent_1px),linear-gradient(90deg,rgba(103,126,214,0.03)_1px,transparent_1px)] bg-[size:40px_40px]"></div>
      </div>
      ...
      >
        ...
        {data.imageUrl && (
          ...
          >
            <div className="absolute -inset-1 bg-gradient-to-r from-slate-600 to-[#677ED6] rounded-3xl opacity-20 blur group-hover:opacity-40 transition duration-1000"></div>
            <div className="relative rounded-2xl overflow-hidden border border-white/10 shadow-2xl bg-[#0A0118]">
              <div className="aspect-video w-full relative">
                <img
                  src={data.imageUrl}
                  alt={data.title}
                  className="w-full h-full object-cover opacity-80 group-hover:scale-105 transition-transform duration-700 ease-out grayscale-[20%]"
                />
                ...
                <div className="absolute bottom-6 left-6 right-6 flex flex-wrap gap-4">
                  {data.meta?.map((item, idx) => (
                    <div
                      key={idx}
                      className="flex items-center gap-3 bg-black/60 backdrop-blur-md border border-white/10 px-5 py-3 rounded-xl shadow-lg hover:border-[#677ED6]/50 transition-colors"
                    >
                      <div className="text-[#677ED6]">
                        {item.icon === "map" && <Network size={18} />}
                        {item.icon === "clock" && <Activity size={18} />}
                        {item.icon === "users" && <Lock size={18} />}
                      </div>
                      <div>
          ...
          <div className="lg:col-span-7 space-y-12">
            ...
            {data.scope && (
              <motion.section variants={fadeInUp}>
                <h3 className="text-xl font-bold mb-6 flex items-center gap-2">
                  <Terminal size={20} className="text-[#677ED6]" /> System Scope
                </h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {data.scope.map((item, index) => (
                    ...
                    >
                      <div className="mt-1">
                        <ShieldAlert className="w-4 h-4 text-[#677ED6]" />
                      </div>
            ...
            <motion.section variants={fadeInUp} className="space-y-6">
              <h3 className="text-xl font-bold flex items-center gap-2">
                <Activity size={20} className="text-[#677ED6]" /> Live Telemetry
              </h3>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {/* Chart 1: Threats Neutralized */}
                ...
                <div className="bg-[#111625] border border-white/10 rounded-2xl p-5 shadow-lg flex flex-col justify-between">
                  ...
                  <div className="h-[180px] w-full">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={encryptionStats} layout="vertical">
              ...
              {data.techStack && (
                ...
                >
                  <div className="absolute top-0 right-0 p-4 opacity-10">
                    <FileCode size={64} className="text-white" />
                  </div>
