...
// --- MOCK DATA FOR CHARTS (Gov/Enterprise Context) ---
const processingData = [
  { name: "Jan", manual: 45, digital: 10 },
  { name: "Feb", manual: 42, digital: 25 },
  { name: "Mar", manual: 38, digital: 45 },
  { name: "Apr", manual: 30, digital: 68 },
  { name: "May", manual: 20, digital: 95 },
  { name: "Jun", manual: 15, digital: 110 },
  { name: "Jul", manual: 8, digital: 145 },
];

const efficiencyStats = [
  { name: "Legacy System", value: 45 }, // Time taken in mins
  { name: "GO Platform", value: 5 }, // Time taken in mins
];
...
const GoProject = () => {
  ...
  // --- FETCH DATA ---
  useEffect(() => {
    // Pointing to "Protfolio/GO"
    const portfolioRef = ref(database, "Protfolio/GO");
    ...
    const unsubscribe = onValue(
      ...
      (snapshot) => {
        if (snapshot.exists()) {
          ...
          console.warn("No data found at 'Protfolio/GO'");
        ...
        <div className="flex flex-col items-center gap-6 z-10">
          <div className="relative">
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <ShieldCheck className="w-6 h-6 text-[#677ED6] opacity-50" />
            </div>
          </div>
  ...
  return (
    <div className="bg-[#0A0118] text-white min-h-screen relative font-sans selection:bg-[#677ED6]/30">
      <Seo
        title={data.title || "Government Operations"}
        description={
          data.overview
      ...
      <div className="fixed inset-0 z-0 overflow-hidden pointer-events-none">
        <div className="absolute top-[10%] left-[30%] w-[800px] h-[800px] bg-[#677ED6]/10 rounded-full blur-[120px]" />
      ...
      >
        ...
        {data.imageUrl && (
          ...
          >
            <div className="absolute -inset-1 bg-gradient-to-r from-[#677ED6] to-indigo-500 rounded-3xl opacity-20 blur group-hover:opacity-40 transition duration-1000"></div>
                  ...
                  {data.meta?.map((item, idx) => (
                    ...
                    >
                      <div className="text-[#677ED6]">
                        {item.icon === "map" && <Users size={18} />}{" "}
                        {/* Defaulting to Users for this context if needed */}
                        {item.icon === "clock" && <Clock size={18} />}
                        {item.icon === "users" && <ShieldCheck size={18} />}
                      </div>
                      <div>
          ...
          <div className="lg:col-span-7 space-y-12">
            ...
            {data.scope && (
              <motion.section variants={fadeInUp}>
                <h3 className="text-xl font-bold mb-6 flex items-center gap-2">
                  <CheckCircle2 size={20} className="text-[#677ED6]" /> Key
                  Objectives
                </h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {data.scope.map((item, index) => (
                    ...
                    >
                      <Zap className="w-5 h-5 text-[#677ED6] shrink-0 mt-0.5" />
            ...
            <motion.section variants={fadeInUp} className="space-y-6">
              <h3 className="text-xl font-bold flex items-center gap-2">
                <Server size={20} className="text-[#677ED6]" /> Performance
                Metrics
              </h3>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {/* Chart 1: Digital Adoption */}
                ...
                <div className="bg-[#111625] border border-white/10 rounded-2xl p-5 shadow-lg flex flex-col justify-between">
                  ...
                  <div className="h-[180px] w-full">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={efficiencyStats} layout="vertical">
